...
- 존댓말 사용''';
  ...
  Future<List<CalendarEvent>> _parsePlanData(DateTime now) async {
    ...
    try {
      final studyData = await _db.doc('users/$_uid/data/study').get();
      if (!studyData.exists) return events;

      final data = studyData.data();
  ...
  Future<List<String>> _fetchMemos(String today) async {
    ...
    try {
      final doc = await _db.doc('users/$_uid/data/study').get();
  ...
  Future<void> addMemo(String memo) async {
    ...
    // ★ 4AM 학습일 경계: 04:00 이전이면 전날 날짜 사용
    final now = DateTime.now();
    final studyDate = now.hour < 4 ? now.subtract(const Duration(days: 1)) : now;
    final today = DateFormat('yyyy-MM-dd').format(studyDate);

    await _db.doc('users/$_uid/data/study').set({
      'dailyMemos': {
  ...
  Future<void> deleteEvent(CalendarEvent event) async {
    switch (event.type) {
      case EventType.study:
        ...
        try {
          await _db.doc('users/$_uid/data/study').update({
