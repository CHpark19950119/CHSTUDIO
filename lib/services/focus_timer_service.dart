...
class FocusTimerService {
  ...
  Future<FocusCycle> endSession() async {
    ...
    final cycle = FocusCycle(
      ...
      date: DateFormat('yyyy-MM-dd').format(_sessionStart!),
  ...
  Future<void> _syncToFirebase(FocusCycle cycle) async {
    try {
      ...
      final record = StudyTimeRecord(
        date: cycle.date,
        totalMinutes:
            (prev?.totalMinutes ?? 0) + cycle.studyMin + cycle.lectureMin,
  ...
  Future<void> _syncLiveProgress() async {
    ...
    try {
      ...
      final dateStr = DateFormat('yyyy-MM-dd').format(_sessionStart!);
