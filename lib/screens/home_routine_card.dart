...
extension _HomeRoutineCard on _HomeScreenState {
      ...
      child: Stack(children: [
        ...
        Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            // 수면 + 식사 미니 행
            if (hasBed || _todayMeals.isNotEmpty || _mealStart != null) ...[
              ...
              if (hasBed)
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: const Color(0xFF6B5DAF).withOpacity(_dk ? 0.08 : 0.04),
                    borderRadius: BorderRadius.circular(10),
                    border: Border.all(color: const Color(0xFF6B5DAF).withOpacity(0.1))),
                  child: Row(children: [
                    const Text('🌙', style: TextStyle(fontSize: 12)),
                    const SizedBox(width: 6),
                    Text('수면 ${_bedTime ?? ''}', style: BotanicalTypo.label(
                      size: 11, weight: FontWeight.w600,
                      color: const Color(0xFF6B5DAF))),
                  ]),
                ),
              // ★ v9→v11: 다회 식사 표시 (탭하여 편집)
              if (_todayMeals.isNotEmpty)
                GestureDetector(
                  onTap: () => _editTimeField('meal', '식사', null),
                  child: Column(children: List.generate(_todayMeals.length, (i) {
                    final m = _todayMeals[i];
                    final typeEmoji = _mealTypeEmoji(m.type);
                    return Container(
                      margin: const EdgeInsets.only(bottom: 4),
                      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                      decoration: BoxDecoration(
                        color: const Color(0xFFFF8A65).withOpacity(_dk ? 0.08 : 0.04),
                        borderRadius: BorderRadius.circular(10),
                        border: Border.all(color: const Color(0xFFFF8A65).withOpacity(0.1))),
                      child: Row(children: [
                        Text(typeEmoji, style: const TextStyle(fontSize: 12)),
                        const SizedBox(width: 6),
                        Text(m.end != null
                            ? '${_mealTypeLabel(m.type)} ${m.start}~${m.end}'
                            : '${_mealTypeLabel(m.type)} 중 ${m.start}~',
                          style: BotanicalTypo.label(
                            size: 11, weight: FontWeight.w600,
                            color: const Color(0xFFFF8A65))),
                        if (m.durationFormatted != null) ...[
                          const Spacer(),
                          Text(m.durationFormatted!, style: BotanicalTypo.label(
                            size: 10, color: const Color(0xFFFF8A65).withOpacity(0.7))),
                        ],
                        const Spacer(),
  ...
  Widget _statusTile({
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(children: [
          // 이모지
          Text(emoji, style: TextStyle(fontSize: active ? 22 : 18)),
          const SizedBox(width: 10),
  ...
  Future<void> _toggleOuting() async {
    ...
    if (_outing == null || (_outing != null && _returnHome != null)) {
      ...
      // 즉시 UI 반영
      if (mounted) setState(() { _outing = timeStr; _returnHome = null; _outingMinutes = null; _locationTracking = true; });
      ...
      if (mounted) await _load(playAnim: false);
