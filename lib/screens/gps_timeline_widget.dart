...
class _GpsTimelineWidgetState extends State<GpsTimelineWidget>
  ...
  Future<void> _loadData() async {
    ...
    try {
      final timeline = await FirebaseService().getBehaviorTimeline(dateStr);
      if (mounted) setState(() { _timeline = timeline; _loading = false; });
      _animCtrl.forward(from: 0);
    } catch (_) {
      if (mounted) setState(() { _timeline = []; _loading = false; });
    }
  }
