...
class _OrderGoalsTabState extends State<OrderGoalsTab> {
  ...
  // ═══ EXAM TICKET SECTION (수험표 관리) ═══
  Widget _examTicketSection(BuildContext context) {
    return orderSectionCard(
      title: '수험표', icon: Icons.confirmation_number_rounded,
      children: _tickets.map((t) {
        ...
        return GestureDetector(
          onTap: () => _showTicketEditDialog(context, t),
          child: Container(
            margin: const EdgeInsets.only(bottom: 8),
            padding: const EdgeInsets.all(14),
            decoration: BoxDecoration(
              color: accent.withOpacity(.04),
            ...
            child: Row(children: [
              ...
              Container(
                ...
                child: Center(child: Text(
                  t.dDayLabel.isNotEmpty ? t.dDayLabel : '📋',
                  style: TextStyle(
                    fontSize: t.dDayLabel.isNotEmpty ? 13 : 18,
              ...
              Expanded(child: Column(
                crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Row(children: [
                    if (t.examDate != null) ...[
                      Icon(Icons.calendar_today_rounded, size: 11, color: OC.text3),
                      const SizedBox(width: 3),
                      Text(t.examDate!, style: const TextStyle(
                        fontSize: 11, color: OC.text3)),
                      const SizedBox(width: 8),
                    ],
                    if (t.examTime != null) ...[
                      Icon(Icons.access_time_rounded, size: 11, color: OC.text3),
                      const SizedBox(width: 3),
                      Text(t.examTime!, style: const TextStyle(
                        fontSize: 11, color: OC.text3)),
                    ],
                  ]),
                  if (t.location != null)
                    Padding(
                      ...
                      child: Row(children: [
                        Icon(Icons.place_rounded, size: 11, color: OC.text4),
                        const SizedBox(width: 3),
                        Expanded(child: Text(t.location!, style: const TextStyle(
              ...
              Icon(Icons.chevron_right_rounded, size: 18, color: OC.text4),
            ]),
          ),
        );
      }).toList(),
  ...
  /// 수험표 수정/삭제 다이얼로그
  Future<void> _showTicketEditDialog(BuildContext context, ExamTicketInfo ticket) async {
    ...
    await showDialog(
      ...
      builder: (c) => AlertDialog(
        ...
        title: Row(children: [
          const Text('📋 수험표 수정', style: TextStyle(
              ...
              final confirm = await showDialog<bool>(
                ...
                builder: (cc) => AlertDialog(
                  shape: RoundedRectangleBorder(
                    borderRadius: BorderRadius.circular(16)),
                  title: const Text('수험표 삭제', style: TextStyle(fontSize: 16)),
                  content: Text('「${ticket.examName}」을(를) 삭제하시겠습니까?'),
                  actions: [
                    TextButton(onPressed: () => Navigator.pop(cc, false),
                      child: const Text('취소')),
                    TextButton(onPressed: () => Navigator.pop(cc, true),
        ...
        actions: [
          ...
          TextButton(onPressed: () async {
            ...
            await ExamTicketService().updateTicket(ticket);
            Navigator.pop(c);
            _loadTickets();
          }, child: const Text('저장')),
        ],
